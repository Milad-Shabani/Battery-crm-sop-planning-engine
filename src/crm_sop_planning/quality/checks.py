"""Quality gate for the planning run."""
...
def run_quality_checks(
    monthly_summary: pd.DataFrame, backtest_accuracy: pd.DataFrame, thresholds: QualityThresholds
) -> QualityReport:
    min_fill_rate = float(monthly_summary["fill_rate"].min()) if not monthly_summary.empty else 1.0

    overall_row = backtest_accuracy[backtest_accuracy["line_id"] == "__OVERALL__"]
    overall_wape = float(overall_row["wape"].iloc[0]) if not overall_row.empty else 0.0

    failures = []
    if min_fill_rate < thresholds.min_fill_rate:
        failures.append(
            f"minimum monthly fill rate {min_fill_rate:.1%} below threshold "
